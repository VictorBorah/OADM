...
{
	$txn = $_GET['txn'];
	if(countData($connAdmApp, "tbl_payments_Roster", " WHERE razorpay_TransactionID='$txn' "))
	{
		$amount = trim(getData($connAdmApp, "tbl_payments_Roster", "paymentAmount", " WHERE razorpay_TransactionID='$txn'   " ));
		$orderID = trim(getData($connAdmApp, "tbl_payments_Roster", "razorpay_OrderID", " WHERE razorpay_TransactionID='$txn'   " ));
		$sid = trim(getData($connAdmApp, "tbl_payments_Roster", "sid", " WHERE razorpay_TransactionID='$txn'   " ));
		$pDate = trim(getData($connAdmApp, "tbl_payments_Roster", "paymentDate", " WHERE razorpay_TransactionID='$txn'   " ));
		
		$fname = trim(getData($connAdmApp, "tbl_students", "std_Fname", " WHERE id='$sid'   " ));
		$mname = trim(getData($connAdmApp, "tbl_students", "std_Mname", " WHERE id='$sid'   " ));
		$lname = trim(getData($connAdmApp, "tbl_students", "std_Lname", " WHERE id='$sid'   " ));
		
		$admCourseID = trim(getData($connAdmApp, "tbl_students", "adm_Course", " WHERE id='$sid'   " ));
		$admCourse = trim(getData($connAdmApp, "tbl_Adm_Courses", "course_Name", " WHERE id='$admCourseID'   " ));
		
		$stdFullName = trim($fname." ".$mname." ".$lname);
		$paymentDate = date_DMY($pDate);
	}
	else header('location: ../index?k9xuRvZ=0');	
}
else
{
	session_destroy();
	setcookie("UKi9GX","",time()-3600,"/",$webRoot);
	setcookie("HJQ8IZYX2KPUQL7DS","",time()-3600,"/",$webRoot);	
	header('location: ../index?k9xuRvZ=0');
}
?>
<!DOCTYPE html>
	 ...
	 <div id="appBx" class="row w-100 ">
		 <div class="row w-100">
		  ...
		  <div class="row w-100 no-margin no-padding frmRow">
			<div class="col-xs-12 col-md-6 col-lg-5 col-centered" style="border:1px dashed #666666;background:#fdfdff;">
				<div class="row">
					<div class="col-xs-12 ">
						<img src= "img/welcome.png" width="200" height="200" class="img-responsive" style="margin:0 auto;"/>	
					</div>	
					<div class="col-xs-12 text-center justify-content-center align-self-center ">
						<h1>Welcome Abroad</h1>
					</div>
					<div class="col-xs-12 text-center justify-content-center align-self-center" style="font-size:16px;font-weight:bold;">
						Hello <?php echo $stdFullName;?>
						<br>
						Welcome to <span style="color:#00a3d9;font-weight:bold;">Orient Flower Junior College</span>.
						<br>
						You have been provisionally admitted to this College.					
					</div>
					<div class="col-xs-12" style="text-align:left;font-size:14px;font-weight:500;margin-top:15px;border-top:1px dashed #666666;border-bottom:1px dashed #666666;padding:10px 0;">
						<b>Your Transaction details:</b><br>
						<span style="font-weight:400;">(<u>You are advised to keep a record of this</u> 👇)</span><br>
						Payment Amount: <span style="color:#d90000"><?php echo $amount;?></span><br>
						Transaction ID: <span style="color:#d90000"><?php echo $txn;?></span><br>
						Digital Receipt No: <span style="color:#d90000"><?php echo $orderID;?></span><br>
						Transaction Date: <span style="color:#d90000"><?php echo $paymentDate;?></span><br>
					</div>
				</div>
			</div>
